...
- No current guarantee of universal transcript-safe host behavior beyond the preview OpenClaw host path, the broker-owned trusted-input ingress path, the local helper redaction contract, and the local helper adapter paths listed in [docs/product/SUPPORTED_HOSTS.md](docs/product/SUPPORTED_HOSTS.md)
