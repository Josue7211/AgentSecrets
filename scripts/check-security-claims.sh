...
grep -Fq "For the certified OpenClaw host path, the repo now also provides host-specific evidence for trusted-input ingress, transcript/log redaction, approval masking, and adapter execution without plaintext leakage." docs/SECURITY_GUARANTEES.md || fail "SECURITY_GUARANTEES missing OpenClaw certification note"
...
grep -Fq "No current guarantee of universal transcript-safe host behavior beyond the certified OpenClaw host path" docs/ARCHITECTURE.md || fail "ARCHITECTURE missing transcript disclaimer"
...
grep -Fq "OpenClaw is the certified host exception only for the documented broker HTTP path" docs/INTEGRATION.md || fail "INTEGRATION missing certified OpenClaw boundary"
grep -Fq "OpenClaw is the first certified external host path in this repo, but only for the documented broker HTTP contract." docs/OPENCLAW.md || fail "OPENCLAW missing certification boundary"
